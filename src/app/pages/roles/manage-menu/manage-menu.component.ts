...
export class ManageMenuComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.addMenuForm.get('module').valueChanges.subscribe(moduleId => {
      if (moduleId) {
        ...
        this.commonHelper.makeRequest(reqObj, 'getMenuModule', false).subscribe(res => {
          if (res.statusCode == 0) {
            this.parentMenuList = res.data;
            !this.parentMenuList ? this.addMenuForm.get('parentMenuId').disable() : this.addMenuForm.get('parentMenuId').enable();
          } else {
  ...
  addMenu() {
    if (this.addMenuForm.valid) {
      ...
      var mandatoryList = (this.addMenuForm.get("mandatoryPermissions").value || []).map(function (m) {
        return (typeof m == "string") ? m : m.permissionCode;
      })

      var optionalList = (this.addMenuForm.get("optionalPermissions").value || []).map(function (m) {
        return (typeof m == "string") ? m : m.permissionCode;
      })
      ...
      this.commonHelper.makeRequest(reqObj, 'addMenu', false).subscribe(res => {
        if (res.statusCode == 0) {
  ...
  editMenu() {
    ...
    if (this.editMenuForm.valid) {
      ...
      var mandatoryNames = (this.editMenuForm.get("mandatoryPermissions").value || []).map(function (m) {
        return (typeof m == "string") ? m : m.displayName;
      });

      var optionalNames = (this.editMenuForm.get("optionalPermissions").value || []).map(function (m) {
        return (typeof m == "string") ? m : m.displayName;
      });

      var mandatoryList = (this.editMenuForm.get("mandatoryPermissions").value || []).map(function (m) {
        return (typeof m == "string") ? m : m.permissionCode;
      })

      var optionalList = (this.editMenuForm.get("optionalPermissions").value || []).map(function (m) {
        return (typeof m == "string") ? m : m.permissionCode;;
      })
      ...
      this.commonHelper.makeRequest(reqObj, 'updateMenu', false).subscribe(res => {
        if (res.statusCode == 0) {

          this.responseMessage = res.data;
          if (this.permissionSearches.length > this.indexSelected) {
            this.permissionSearches[this.indexSelected].sequence = reqObj.sequence;
            this.permissionSearches[this.indexSelected].menuCode = reqObj.menuCode;
            this.permissionSearches[this.indexSelected].caption = reqObj.caption;
            this.permissionSearches[this.indexSelected].menuId = reqObj.menuId;
          }

          this.selectedOptional = [];
          this.selectedMandatory = [];

          this.selectedMandatoryDisplayNames = this.selectedMandatoryDisplayNames.concat(mandatoryNames);
          this.selectedOptionalDisplayNames = this.selectedOptionalDisplayNames.concat(optionalNames);
  ...
  openEditForm(menuId, index) {
    ...
    this.commonHelper.makeRequest(reqObj, 'getMenuDetail', false).subscribe(res1 => {
      if (res1.statusCode == 0) {
        ...
        this.commonHelper.makeRequest({ token: localStorage.getItem('authToken') }, 'getPermission', false).subscribe(res2 => {
          if (res2.statusCode == 0) {
            this.optionalPermissionsList = res2.data;
            this.mandatoryPermissionsList = res2.data;

            var selectedOptionalItems = this.editFormData.permissions.filter(function (o) {
              return o.isRequired == "NO";
            })

            this.selectedOptionalCodes = selectedOptionalItems.map(function (o) {
              return o.permissionCode;
            })

            this.selectedOptionalDisplayNames = selectedOptionalItems.map(function (o) {
              return o.displayName;
            })

            var selectedMandatoryItems = this.editFormData.permissions.filter(function (o) {
              return o.isRequired == "YES";
            })

            this.selectedMandatoryCodes = selectedMandatoryItems.map(function (o) {
              return o.permissionCode;
            })

            this.selectedMandatoryDisplayNames = selectedMandatoryItems.map(function (o) {
              return o.displayName;
            })

            var that = this;

            this.mandatoryPermissionsList = this.mandatoryPermissionsList.filter(function (o) {
              return (that.selectedMandatoryCodes.indexOf(o.permissionCode) <= -1 && that.selectedOptionalCodes.indexOf(o.permissionCode) <= -1)
            })

            this.optionalPermissionsList = this.optionalPermissionsList.filter(function (o) {
              return (that.selectedMandatoryCodes.indexOf(o.permissionCode) <= -1 && that.selectedOptionalCodes.indexOf(o.permissionCode) <= -1)
            })
        ...
        this.editMenuForm.patchValue({ sequence: this.editFormData.sequence });
        this.editMenuForm.patchValue({ basePath: this.editFormData.basePath });
        this.editMenuForm.patchValue({ relativePath: this.editFormData.relativePath });
        this.editMenuForm.patchValue({ apiDetails: this.editFormData.apiDetails });
        this.editMenuForm.patchValue({ menuCode: this.editFormData.menuCode });

        this.editFormData.menuCode ? this.editMenuForm.get("menuCode").disable() : this.editMenuForm.get("menuCode").enable();

        this.editMenuForm.patchValue({ engineCode: this.editFormData.engineCode });
        this.editMenuForm.patchValue({ parentMenu: this.editFormData.parentId });
  ...
  addMenuClicked() {
    ...
    this.commonHelper.makeRequest({ token: localStorage.getItem('authToken') }, 'getPermission', false).subscribe(res => {
      if (res.statusCode == 0) {
  ...
  onSelectingSingleMandatoryPermission(event) {
    this.optionalPermissionsList = this.optionalPermissionsList.filter(function (d) {
  ...
  onDeSelectingSingleMandatoryPermission(event) {
    if (typeof event == "string") {
      ...
      this.optionalPermissionsList = this.optionalPermissionsList.concat([event]);
    }
  }
  ...
  onDeSelectingAllMandatoryPermission(event) {
    this.optionalPermissionsList = this.optionalPermissionsList.concat(this.mandatoryPermissionsList);
  }

  onSelectingSingleOptionalPermission(event) {
    this.mandatoryPermissionsList = this.mandatoryPermissionsList.filter(function (d) {
      return d.permissionCode != event.permissionCode;
    });
  }
  ...
  onDeSelectingSingleOptionalPermission(event) {
    if (typeof event == "string") {
      ...
      this.mandatoryPermissionsList = this.mandatoryPermissionsList.concat([event]);
    }
  }
  ...
  onDeSelectingAllOptionalPermission(event) {
    this.mandatoryPermissionsList = this.mandatoryPermissionsList.concat(this.optionalPermissionsList);
  }
